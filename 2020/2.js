...
var pwords = data.map(function(d) {   
   return d.split(' ')
 }).filter((d) => {        
    var range = d[0].split('-');    
    var replace = d[1].charAt(0);
    var re = new RegExp(replace, 'g');  
    var l2 = d[2].length - (d[2].replace(re, '').length);   
    return l2 >= range[0] && l2 <= range[1]; 
 }).length;

var pwords2 = data.map(function(d) {   
    return d.split(' ')
  }).filter((d) => {        
     var range = d[0].split('-');    
     var replace = d[1].charAt(0);
     var pos1 = d[2].charAt(range[0]-1);
     var pos2 = d[2].charAt(range[1]-1);
     return (pos1 != pos2) && (pos1 == replace || pos2 == replace);
  }).length;
